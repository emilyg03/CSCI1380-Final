...
fps = 15         # FPS to control how fast game runs
...
apple = pygame.Rect(apple_position[0], apple_position[1], apple_size, apple_size)   # Rectangle for the apple at the random position
...
while running:
    for event in pygame.event.get():
        if event.type == pygame.QUIT: # If player clicks window close button
            ...
            if event.key == pygame.K_UP and direction != 'DOWN':       # Down
                direction = 'UP'
            elif event.key == pygame.K_DOWN and direction != 'UP':     # Up
                direction = 'DOWN'
            elif event.key == pygame.K_LEFT and direction != 'RIGHT':  # Right
                direction = 'LEFT'
            elif event.key == pygame.K_RIGHT and direction != 'LEFT':  # Left
                direction = 'RIGHT'
...
game_screen.blit(game_over_text, (screen_width // 2 - game_over_text.get_width() // 2, screen_height // 2))     # Location, center
